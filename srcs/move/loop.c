...
/*   Updated: 2021/03/25 17:57:10 by odroz-ba         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_view(t_cub *cub)
{
	...
	if (cub->key.al)
		cub->player.agl_hor -= cub->delta;
	if (cub->key.ar)
		cub->player.agl_hor += cub->delta;
	if (cub->key.ad)
	{
		rs = cub->player.agl_vrt + 0.5 * (cub->delta * SENSIBILITY);
...
static void	ft_move(t_cub *cub)
{
	...
	if (cub->pars->map[pos_i.y][pos_i.x] == '1' && new_pos.z < S_W + 0.2)
		new_pos.z = S_W + 0.2;
