...
/*   Updated: 2021/03/16 14:51:03 by odroz-ba         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */

#include "cub3D.h"

static int	ft_is_we_wall(t_ptr *ptr, unsigned int x)
{
	unsigned int	y;
...
static int	ft_is_ea_wall(t_ptr *ptr, unsigned int x)
{
	unsigned int	y;
...
void		ft_create_plans_x(t_ptr *ptr)
{
	...
	while (++x < ptr->pars->nbr_map.x)
	{
		...
		if (ft_is_we_wall(ptr, x))
		{
			...
			// ptr->pars->plans_we[i].rs = ft_calc_rs(ptr, &ptr->pars->plans_we[i]);
		}
		else
			ptr->pars->plans_we[i].d = 0;
		if (ft_is_ea_wall(ptr, x))
		{
			...
			// ptr->pars->plans_ea[i].rs = ft_calc_rs(ptr, &ptr->pars->plans_ea[i]);
		}
		else
			ptr->pars->plans_ea[i].d = 0;
	}
